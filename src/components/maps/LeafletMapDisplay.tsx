...
const LeafletMapDisplay: FC<LeafletMapDisplayProps> = ({ centerCoordinates, facilities, userLocationName }) => {
  ...
  useEffect(() => {
    if (!mapContainerRef.current || !centerCoordinates) {
      return;
    }
    ...
    if (userMarkerRef.current) {
      ...
      userMarkerRef.current = L.marker([centerCoordinates.latitude, centerCoordinates.longitude], {
         icon: L.icon({ // Custom user icon
          ...
          popupAnchor: [1, -34],
          shadowSize: [41, 41],
          // You might need to adjust filter for a different color if using the default icon
        }),
      }).addTo(mapInstanceRef.current);
    }
    ...
    facilities.forEach(facility => {
      // Use facility.coordinates if available, otherwise skip (or geocode if implemented)
      ...
      if (facility.coordinates) {
        ...
        // Fallback: if no explicit coords, try to derive from a known location for mock data, or skip
        // For this example, we assume some mock facilities might not have coordinates and will be skipped.
        // In a real app, you'd ensure all facilities have coordinates or geocode them.
      ...
      if(facilityCoords) {
        ...
        const marker = L.marker(facilityCoords, { icon: facilityIcon }).addTo(mapInstanceRef.current!);
        ...
        marker.on('click', () => {
             // router.push(detailLink); // Alternative navigation, popup link is often better UX
        });
    ...
    return () => {
      if (mapInstanceRef.current) {
        // mapInstanceRef.current.remove(); // This can cause issues if other effects try to use it
        // mapInstanceRef.current = null; // Better to leave it to be garbage collected if component unmounts fully
      }
    };
  }, [centerCoordinates, facilities, router, userLocationName]); // Added userLocationName

  if (!centerCoordinates) {
    return (
      <div className="flex items-center justify-center h-64 bg-muted rounded-lg shadow">
